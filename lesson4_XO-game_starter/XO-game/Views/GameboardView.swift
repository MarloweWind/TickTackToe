...
public class GameboardView: UIView {
    ...
    public func performMove(at position: GameboardPosition?){
        onSelectPosition?(position)
    }
    ...
    public func isUserMark(at position: GameboardPosition, by player: Player) -> Bool {
        guard !self.canPlaceMarkView(at: position) else { return false }
        
        if markViewForPosition[position] == player.markViewPrototype {
            return true
        }
